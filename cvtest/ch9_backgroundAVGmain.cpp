...
#pragma endregion initparam

#pragma region ifarg<=4
...
#pragma endregion ifarg<=4

#pragma region ifarg>4 高阶调节参数
...
#pragma endregion ifarg>4 高阶调节参数
...
#pragma region ifcapture==true
      ...
      cvNamedWindow( "Raw", 1 );
		...
		cvNamedWindow("rectimg",1);
        int i = -1;
        
        for(;;)
        {
#pragma region in "for;;" loop

    			if(!pause){
#pragma region getFrame
...
#pragma region Firsttime ii==0
				...
				rectimg = cvCreateImage(cvGetSize(rawImage),rawImage->depth,rawImage->nChannels);//llf
				...
				ch[1] = true;
				ch[2] = true;
#pragma endregion Firsttime ii==0			
			}
			//If we've got an rawImage and are good to go:                
        	if( rawImage )
        	{
#pragma region raw img exist

				cvCopy(rawImage,rectimg);
				...
				if(i >= endcapture) {
					...
					int num=1;
					cvconnectedComponents(ImaskAVGCC,1,4.0,&num,rect);
					printf("find rect:(%d,%d,%d,%d) i:%d\n",rect->x,rect->y,rect->width,rect->height,i);
					
					//FIND FOREGROUND BY CODEBOOK METHOD
#pragma region codebook method
#if 1
...
#endif
#pragma endregion codebook method
...
#pragma region display
				...
				cvSetImageROI(ImaskAVGCC,*rect);
				cvAddS(ImaskAVGCC,cvScalar(100),ImaskAVGCC);
				cvResetImageROI(ImaskAVGCC);
				cvShowImage( "AVG_ConnectComp",ImaskAVGCC);

   				cvShowImage( "ForegroundAVG",ImaskAVG);
							
				cvSetImageROI(rectimg,*rect);
				cvAddS(rectimg,cvScalar(50),rectimg);
...
#pragma region pause to inputtt
...
#pragma endregion pause to inputtt		
  #pragma endregion raw img exist         
			}
	#pragma endregion in "for" loop
		}		
#pragma region distroy mem
      ...
      cvDestroyWindow( "Raw" );
		...
		delete [] cB;
#pragma endregion distroy mem
 #pragma endregion ifcapture==true 
  }
